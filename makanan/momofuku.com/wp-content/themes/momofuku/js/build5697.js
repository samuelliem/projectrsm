...
var contact = function () {
	...
	var markers = [];
	var marker, i, infoWindow, data;

	function _init() {
    	google.maps.event.addDomListener(window, 'load', _googleMap());
	}
   	...
   	function _googleMap(){
        ...
        cell.each(function () {
            ...
            locations.push([place, lat, lng, address, web, contact, map, email]);    

        });

        function _setMarkers(map, loc) {
         
			var image = {
				url: '/app/themes/momofuku/img/icon-pin.png', 
			};

			var url = window.location.hostname;
		
			for (i = 0; i < loc.length; i++) {
				var textMap = 'get directions';
				var textWeb = 'visit website';
				var	textBtn = 'contact us';
				var city = loc[i];
				var pathArray = city[5].split( '/' );
				var link = pathArray[2];
				var myLatLng = new google.maps.LatLng(city[1], city[2]);
				
				if(url != link && city[5] != "none" && city[7] === ""){
					data = '<div class="tooltip"><h4>'+city[0]+'</h4><p>'+city[3]+'</p><div class="map-web"><a class="map" href="https://www.google.com/maps/place/'+city[6]+'" target="_blank">'+textMap+'</a>&#124;<a class="web" href="'+city[4]+'" target="_blank">'+textWeb+'</a></div><a class="contact" href="'+city[5]+'" target="_blank">'+textBtn+'</a></div>';
				} else if (url == link && city[5] != "none" && city[7] === "") {
					data = '<div class="tooltip"><h4>'+city[0]+'</h4><p>'+city[3]+'</p><div class="map-web"><a class="map" href="https://www.google.com/maps/place/'+city[6]+'" target="_blank">'+textMap+'</a>&#124;<a class="web" href="'+city[4]+'" target="_blank">'+textWeb+'</a></div><a class="contact" href="'+city[5]+'">'+textBtn+'</a></div>';
				} else if (city[7] !== ""){
					data = '<div class="tooltip"><h4>'+city[0]+'</h4><p>'+city[3]+'</p><div class="map-web"><a class="map" href="https://www.google.com/maps/place/'+city[6]+'" target="_blank">'+textMap+'</a>&#124;<a class="web" href="'+city[4]+'" target="_blank">'+textWeb+'</a></div><a class="contact" href="mailto:'+city[7]+'">'+textBtn+'</a></div>';
				} else {
					data = '<div class="tooltip"><h4>'+city[0]+'</h4><p>'+city[3]+'</p><div class="map-web"><a class="map" href="https://www.google.com/maps/place/'+city[6]+'" target="_blank">'+textMap+'</a>&#124;<a class="web" href="'+city[4]+'" target="_blank">'+textWeb+'</a></div></div>';
				}
				...
				_openMarkers(marker, infoWindow);
        		markers.push(marker);
        ...
        function _myClick(id){
            google.maps.event.trigger(markers[id], 'click');
        }

		var mapOptions = {
			//disableDefaultUI: true,    
            center: new google.maps.LatLng(40.7291757,-73.98439250000001),
            zoom: 14,
            scrollwheel: false,
            zoomControl: true,
        ...
        function _openMarkers(marker, infoWindow){
...
(function () {
    ...
    var $item = $('section.grid-elements .inner > div.grid-item');
    var $title = $('section.title').height();
    var $menu_items = $('#sub > div.nav-section > ul.sub-menu li.menu-item-object-page');
    var $text = $('p.item-text');
    var $textH = $text.height();
    var $type = $('.overlay h4');
    var $box = $('.info');
    var $caption = $('div.caption');
    var $menu = $('div.first-level').height();
    var $featured = $('#home section.featured').find('.inner-item');
    var $bg = $('#home div.featured').find('.inner-default > div.top');
    var $eleMobile = $('div.trigger > ul').length;
    var $cat_lgt = $('#cat').outerWidth();
    var $cat_li = $('.dropdown-menu > li > a');

    $(function() {

        var nomTemplate = $('body').attr('data-template').replace('-','_');
        //call les bonnes fonctions relatives à une page
        if (nomTemplate !== '' && window[nomTemplate] && typeof (window[nomTemplate].init) === 'function') {
            window[nomTemplate].init();
        }

        if ($('body').hasClass('post-type-archive-press') || $('body').hasClass('tax-years') || $('body').hasClass('tax-categories') || $('body').hasClass('tax-types')){
           $('li.press').addClass('current_page_item');
           $('li.navigation').addClass('current_page_parent');
        ...
        _desktop();
        _press("#cat","#cat-drop");
        _press("#year","#year-drop");
        _press("#type","#type-drop");
        _press("#cat-mo","#cat-drop-mo");
        _press("#year-mo","#year-drop-mo");
        _press("#type-mo","#type-drop-mo");
        _press_mobile();
        _common();
        _menu();
        _resize();
        ...
        _404();
        _securePage($winH);
        _equalblocks('.small .bottom');
        _equalblocks('.normal .bottom');
        _equalblocks('ul.grid > li.cell');
        _filtering();
        _iosFix();
        // resizeBackground();
	...
	function _common(){
		...
		// White fade effect on page loading
        $('body').css('min-height', $winH);
        $('#protected').css('min-height', $winH-100);
        $('span.icon-ctrl').css('transform', 'rotate(180deg)');
        $item.each(function(){
            var ovH = $(this).height();
            var capH = $(this).find('div.caption').outerHeight();
            TweenLite.to($(this).find($caption), 0.1,{css:{bottom:-capH}});
            TweenLite.to($(this).find('.info-top'), 0.1,{css:{height:ovH+'px'}});
        });
    ...
    function _desktop(){
        ...
        TweenLite.to($('#banner') ,.5, {y:"-50", ease:Power2.easeOut});
        TweenLite.to($text , 0, {autoAlpha: 0, height:0, clearProps:"display"});

    }

    function _securePage(winH){

        $('#protected').css('min-height', winH-100);

    }

    function _bannerResp(winW){
        // Remove arrow on mobile if theres no submenu
        // console.log($eleMobile);
        if ($eleMobile < 1 && winW < 768) {
            $('#banner').hide();
        } else {
            $('#banner').show();
        }
    ...
    function _press(cat,drop){

        $(cat).bind('click', function(e){
            e.preventDefault();
            var $this = $(this), $content = $(drop), $arrow = $this.find('span.icon-ctrl'), $siblings = $('.close').siblings('ul'), $arrow_siblings = $('.close').find('span.icon-ctrl');
            ...
            if (!$this.hasClass("close")){
                ...
                $arrow_siblings.css('transform', 'rotate(180deg)');
                $siblings.prev('a').removeClass('close');
    ...
    function _press_mobile(){

        var current_cat = $('#cat-drop .selected > a').text();
        var current_year = $('#year-drop .selected > a').text();
        var current_type = $('#type-drop .selected > a').text();

        if(current_cat){
            $('#cat > span.top-item').text(current_cat);
        } else {
            $('#cat > span.top-item').text('All Categories');
        }

        if(current_year){
            $('#year > span.top-item').text(current_year);
        } else {
            $('#year > span.top-item').text('All Years');
        }

        if(current_type){
            $('#type > span.top-item').text(current_type);
        } else {
            $('#type > span.top-item').text('All Types');
        }

        var current_cat_mo = $('#cat-drop-mo .selected > a').text();
        var current_year_mo = $('#year-drop-mo .selected > a').text();
        var current_type_mo = $('#type-drop-mo .selected > a').text();

        if(current_cat_mo){
            $('#cat-mo > span.top-item').text(current_cat_mo);
        } else {
            $('#cat-mo > span.top-item').text('All Categories');
        }

        if(current_year_mo){
            $('#year-mo > span.top-item').text(current_year_mo);
        } else {
            $('#year-mo > span.top-item').text('All Years');
        }

        if(current_type_mo){
            $('#type-mo > span.top-item').text(current_type_mo);
        } else {
            $('#type-mo > span.top-item').text('All Types');
        }
	...
	function _menu(){
		...
		// our company sub menu
		$("li.navigation > a").bind('click', function(e){ 
            e.preventDefault();
            TweenLite.to("#sub .nav-section ul.sub-menu > li", 1, {left:"632px"});
            if (!$(this).hasClass("close")){
              	$(this).addClass("close");
                $(this).find('span').addClass('close');
                TweenLite.to($('#sub') ,.5, {left:"200px", ease:Power2.easeOut});

                if ($("li.resto > a").hasClass("close")) {
               		TweenLite.to($('#subResto') ,.01, {left:"-120px", ease:Power2.easeOut});
               		$("li.resto > a").removeClass("close");
               		$("li.resto > a").find("span").removeClass("close");                	
                }
            } else {
              	$(this).removeClass("close");
                $(this).find('span').removeClass('close');
              	TweenLite.to($('#sub') ,.5, {left:"-120px", ease:Power2.easeOut});
            }
        });

        $("#sub-mobile").bind('click', function(e){ 
            e.preventDefault();
            var $this = $(this), $content = $("#mobile-nav");
		...
		// our restaurants sub menu
		$(".middle-section #menu li.resto > a").bind('click', function(e){ 
            e.preventDefault();
            TweenLite.to("#subResto .nav-section ul.sub-menu > li", 1, {left:"632px"});
            if (!$(this).hasClass("close")){ 
              	$(this).addClass("close");
                $(this).find('span').addClass('close');
              	TweenLite.to($('#subResto') ,.5, {left:"200px", ease:Power2.easeOut});

              	if ($("li.navigation > a").hasClass("close")) {
               		TweenLite.to($('#sub') ,.01, {left:"-120px", ease:Power2.easeOut});
               		$("li.navigation > a").removeClass("close");
               		$("li.navigation > a").find("span").removeClass("close");                	
                }

            } else {
              	$(this).removeClass("close");
                $(this).find('span').removeClass('close');
              	TweenLite.to($('#subResto') ,.5, {left:"-120px", ease:Power2.easeOut});
            }
        });


        $('#mobile-section li.resto a').text('Restaurants');
        $('#mobile-section li.ventures a').text('Ventures');

	}

    function _animItems() {

        TweenLite.to($('#banner') ,.5, {y:"0", ease:Power2.easeOut, delay:0.4});
        ...
        $item.on('mouseenter', function(e){
            ...
            $(this).each(function(){
                ...
                TweenLite.to($(this).find('.info-top'), 0.4,{css:{height:newH+'px'}});
                TweenLite.to($(this).find($text), 0.4,{css:{height:'auto'}});

                TweenLite.to($(this).find($text) , 0.4, {autoAlpha: 1, display:'block'});
                TweenLite.to($(this).find($caption), 0.4,{css:{bottom:0}});
        ...
        $item.on('mouseleave', function(e){
            ...
            e.preventDefault();
                ...
                TweenLite.to($(this).find('.info-top'), 0.1,{css:{height:ovH+'px'}});
                TweenLite.to($(this).find($text), 0.1,{css:{height:0}});

                TweenLite.to($(this).find($text) , 0, {autoAlpha: 0, clearProps:"display"});
                TweenLite.to($(this).find($caption), 0.1,{css:{bottom:-capH}});
        ...
        if(page != "by-location"){
            ...
            // adds class full-click if only 1 item
            $('div.caption').each(function(){
                var items = $(this).find('ul > li').length;
                if (items == 1){
                    var link = $(this).find('a').attr('href');
                    $(this).parent().addClass('full-click');
                }
            });

            // Whole block clickable if only 1 item
            $('.full-click').on('click', function(){
                var link = $(this).find('a').attr('href');
    ...
    function _404(){
        ...
        $(window).bind('load resize', function(){
            var $menu = $('div.first-level').height();
            var $winH = $(window).height();
            $('body.error404').css('min-height', $winH);
            if ($winW < 768){
               $('body.error404').css('min-height', $winH-$menu); 
            }
        });
	...
	function _resize(){
		// Closes mobile menu on resize
	    if ($winW > 767){
	    	$('#sub-mobile').removeClass('close');
	    	$('#mobile-nav').css('height', 0);
            $cat_li.css('width','100%');
	    } else if($winW < 768){
			$('#home .featured .inner-box').css('height', $winH);
            $cat_li.css('width',$cat_lgt);
	    }
	}

    function _featMobile(){
        // Resize Featured Post to fit mobile screen
        if ($winW < 768){
            $('#mobile-nav').css('max-height',$winH);
            $featured.css('margin-top', -$menu);
            $featured.height($winH);
            $bg.height($winH-($menu + 57 + $title));
            $cat_li.css('width',$cat_lgt);
        } 
    ...
    function _submenu(){
        ...
        $('#sub').click(function(event){ 
            TweenLite.to($('#subResto') ,.0, {left:"-120px", ease:Power2.easeOut});
            event.stopPropagation();
            
        });

        $('#subResto').click(function(event){
            TweenLite.to($('#sub') ,.0, {left:"-120px", ease:Power2.easeOut});
            event.stopPropagation();
            
        });

        $('#sidemenu #menu').click(function(event){
            event.stopPropagation();
        });

        $("div.trigger span.mobile").bind('click', function(e){ 
            e.preventDefault();
            var $this = $(this), $content = $("#banner ul.sub-menu"), $arrow = $('span.mobile');
    ...
    function _filtering(){
        ...
        var url = root + window.location.pathname;
        ...
        for ( var i = 0 ; i < params.length; i++){
            ...
            switch (param[0]){
                case 'categories': var ctg = param[1];
                break;
                case 'year': var year = param[1];
                break;
                case 'type': var type = param[1];
                break;
            }
        }
        ...
        $('#cat-drop li > a').on('click', function(e){
            e.preventDefault();
            $(this).each(function(){
                var data = $(this).parent().attr('class');
                params = parseQuery('categories', data);
                window.location = url + params;
            });
        });

        $('#year-drop li > a').on('click', function(e){
            e.preventDefault();
            $(this).each(function(){
                var data = $(this).parent().attr('class');
                params  = parseQuery('years', data);
                window.location = url + params;
            });
        });

        $('#type-drop li > a').on('click', function(e){
            e.preventDefault();
            $(this).each(function(){
                var data = $(this).parent().attr('class');
                params = parseQuery('type', data);
                window.location = url + params;
            });
        });

        $('#cat-drop-mo li > a').on('click', function(e){
            e.preventDefault();
            $(this).each(function(){
                var data = $(this).parent().attr('class');
                params = parseQuery('categories', data);
        ...
        $('#year-drop-mo li > a').on('click', function(e){
            e.preventDefault();
            $(this).each(function(){
                var data = $(this).parent().attr('class');
                params = parseQuery('years', data);
        ...
        $('#type-drop-mo li > a').on('click', function(e){
            e.preventDefault();
            $(this).each(function(){
                var data = $(this).parent().attr('class');
                params  = parseQuery('type', data);
        ...
        function parseQuery(taxonomy, data) {
            
            var params = '';
            var pattern = new RegExp(taxonomy+'=[0-9a-z%\-]+(&|$)');
            var element = taxonomy+'=' + data;
            // console.log(element);
            if(data !== undefined){
                params = '?';
                if(window.location.search.indexOf(taxonomy+'=') > -1) {
                    params = window.location.search.replace(pattern, element + '&');
                } else { 
                    params = params + window.location.search + '&' + element;
                }
            } else {
                params = window.location.search.replace(pattern, '');
            }
    ...
    function _iosFix(){
        ...
        if(/iPhone|iPod|Android|iPad/.test(window.navigator.platform)){
            ...
            $('.first-level').css('position', 'absolute');
            })

            .on('blur', 'textarea,input,select', function(e) {
            $('.first-level').css('position', 'fixed');
            });
    ...
    function _GAtracking(){

        $("span.icon-twitter").on("click" , function(){
            __gaTracker('send', 'event', 'Social', 'Click', 'Twitter');
        });
        $("span.icon-facebook").on("click" , function(){
            __gaTracker('send', 'event', 'Social', 'Click', 'Facebook');
        });
        $("span.icon-instagram").on("click" , function(){
            __gaTracker('send', 'event', 'Social', 'Click', 'Instagram');
        });
        $("span.icon-tumblr").on("click" , function(){
            __gaTracker('send', 'event', 'Social', 'Click', 'Tumblr');
        });
        $("#menu-bottom .reserve").on("click" , function(){
            __gaTracker('send', 'event', 'Reservation', 'Click', '');
        });
...
}(window));var other_ventures = function () {
	"use strict";
...
var by_location = function () {
	...
	function _googleMap(){
        ...
        var cell = $('ul.cap-items > li').find('a.map');
        var locations = [];
        ...
        cell.each(function () {
        	...
        	map = $(this).data('map'),
        	email = $(this).data('email'),
        	location_status = $(this).data('locationstatus');
        	
            locations.push([name, lat, lng, address, web, contact, map, email, location_status]);  
        ...
        function _setMarkers(map, loc) {
			...
			for (i = 0; i < loc.length; i++) {
	            ...
	            var pathArray = city[5].split( '/' );
				var link = pathArray[2];
				var loc_stat = loc[i][8];

				if (loc_stat == "open") { 
					image = { url: '/app/themes/momofuku/img/icon-pin.png' }; 
				} else { 
					image = {  url: '/app/themes/momofuku/img/icon-coming-soon.png' }; 
				}
		...
		if (navigator.geolocation) {
			...
			// need to set a default for NYC if they deny geolocation
			 
			var default_location = { coords: { latitude: 40.7291757, longitude:-73.98439250000001}, timestamp: Date.now() }
			showPosition(default_location)
		}
		...
		function showPosition(position) {
			...
			for(var i = 0; i < locations.length; i++) 
			{
				...
				var dist = Haversine( locations[ i ][1], locations[ i ][2], lat, lon );
			...
			var latlng = new google.maps.LatLng( locations[ closest][1], locations[ closest][2] );    
...
*/
		...
		//	var infowindow = new google.maps.InfoWindow( { content: contentString } );  
  		...
  		$('a.map').on('click', function(e){
  			...
  			var lat = $(this).data('lat');
        	var lng = $(this).data('lng');
        	// console.log(num,lat,lng);
		    map.panTo(new google.maps.LatLng(lat, lng));
	        _myClick(num); 
		    // map.setZoom(12);
		    $('html, body').animate({
	            scrollTop: $("#map-canvas").offset().top - 120
	        }, 500);
  		});
